...
model = joblib.load(f'{working_dir}/model/model.joblib')
scaler = joblib.load(f'{working_dir}/model/scaler.joblib')
ohe = joblib.load(f'{working_dir}/model/ohe.joblib')
le = joblib.load(f'{working_dir}/model/le.joblib')
...
def predict(data: Data):
    ...
    # Convert INR to PKR (1 INR ≈ 0.32 PKR)
    inr_price = int(prediction)
    pkr_price = int(inr_price * 0.32)

    return JSONResponse(status_code=200, content={
        "Prediction_INR": inr_price,
        "Prediction_PKR": pkr_price
    })
